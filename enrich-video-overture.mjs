...
const DEFAULT_CITY_MATCH_RADIUS_KM = 35;
const AIRPORT_CITY_MATCH_RADIUS_KM = 90;
...
const LOCATION_NOISE_WORDS = new Set([
  ...
  'cinematic', 'live', 'full', 'complete', 'entire', 'long',
  ...
  'central', 'collection', 'driving', 'evening', 'morning',
  'afternoon', 'guide', 'part', 'chapter', 'section',
...
const AMBIGUOUS_LOCATION_WORDS = new Set([
  'aerial', 'cat', 'central', 'collection', 'country', 'driving',
  'england', 'evening', 'food', 'full', 'guide', 'in', 'island',
  'long', 'market', 'morning', 'night', 'nyc', 'part', 'preview',
  'roman', 'room', 'tour', 'turkey', 'turkiye', 'türkiye', 'uk',
  'video', 'walk', 'walking', 'world'
]);
...
function stripGeneratedGeo(point = {}) {
  const {
    canonicalName,
    sourceVerified,
    geoVerified,
...
function cleanRoutePointsForEnrichment(video) {
  return (
    ...
    .map(stripGeneratedGeo);
}

function isMeaningfulCandidate(name, video) {
  ...
  const tokens = distinctiveTokens(name)
    .filter(
      (token) =>
        ...
        !AMBIGUOUS_LOCATION_WORDS.has(token)
    );

  return tokens.length > 0;
}

function buildCandidatesForVideo(video, routePoints) {
...
function hotelCategoryCondition(placeSchema) {
  if (placeSchema.basicCategory) {
    return "basic_category IN ('hotel', 'resort_hotel', 'boutique_hotel')";
  }

  if (placeSchema.taxonomy) {
    return "taxonomy.primary IN ('hotel', 'resort_hotel', 'boutique_hotel')";
  }

  if (placeSchema.categories) {
    return "categories.primary IN ('hotel', 'resort_hotel', 'boutique_hotel')";
  }
...
function queryCandidateRecords(
  ...
  const routeSearchRadiusKm = Math.max(
    ...candidates.map(
      (candidate) =>
        maximumCityDistanceMeters(candidate) /
        1_000
    )
  );
  ...
  const divisionSql = `${duckDbPreamble()}
    ...
    WHERE ${bboxCondition(bounds, false)}
      AND country = ${
...
function maximumCityDistanceMeters(candidate) {
  return /\b(?:airport|terminal)\b/iu
    .test(candidate.name)
    ? AIRPORT_CITY_MATCH_RADIUS_KM * 1_000
    : DEFAULT_CITY_MATCH_RADIUS_KM * 1_000;
}
...
function applyRecordToPoint(
  ...
  return {
    ...
    verified:
      candidate.kind === 'title' ||
      candidate.point.sourceVerified === true ||
...
function isMostlyLatinName(value) {
  const letters = [...cleanText(value)]
...
function preferredHotelName(primary, namesJson) {
  const names = flattenNameValues(
    primary,
    namesJson
  );
...
function hotelsNearAnchor(
  ...
  const hotels = records
    .map((record) => {
      ...
      if (
        /\b(?:clinic|driving school|pharmacy|restaurant|tyre)\b/iu
          .test(name)
      ) {
        return null;
      }
...
async function main() {
      ...
      if (anchor) {
        enrichedVideo.geo = {
          ...
          verificationVersion:
            ENGINE_VERSION,
          cityDistanceMeters:
            anchor.cityDistanceMeters,
          updatedAt:
            timestamp
        };
      } else {
        delete enrichedVideo.geo;
      }
  ...
  console.log(
    args.write
      ? `Overture evidence written to ${args.outputPath || args.inputPath}`
