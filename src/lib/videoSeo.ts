...
export const VIDEO_CONTENT_VERSION = 10;
...
export type VideoNearbyHotel = {
  ...
  bookingUrl?: string;
  verified?: boolean;
  cityContextVerified?: boolean;
};

export type VideoGeo = {
  latitude: number;
  longitude: number;
  placeId?: string;
  precision?: string;
...
const weakWords = new Set([
  ...
  'suburban','vibe','vibes','natural','record','breaking','central',
  'must','see','world','worlds','why','how','this','that',
  'feels','feel','looks','look','hour','hours','hr','hrs',
  'minute','minutes','complete','true','full','long','entire',
...
const genericLocationLabels = new Set([
  'city walk', 'complete tour', 'complete video', 'complete walk',
  'entire tour', 'entire video', 'entire walk', 'full tour', 'full video',
  'full walk', 'long', 'tour', 'video', 'walk', 'walking tour',
  ...
  'tour đi bộ', 'preview', 'introduction', 'intro', 'start', 'opening',
  'ending', 'outro', 'highlights', 'welcome', 'in this video',
  'video highlights', 'video preview', 'preview highlights'
]);

const genericLabelWords = new Set([
  'beginning', 'chapter', 'city', 'complete', 'end', 'ending', 'entire',
  'full', 'highlights', 'intro', 'introduction', 'long', 'opening', 'outro',
  'part', 'preview', 'section', 'start', 'tour', 'video', 'walk', 'walking',
  'welcome'
]);

const genericDurationPattern = /^(?:\d+(?:[.,]\d+)?\s*)?(?:s|sec|secs|second|seconds|min|mins|minute|minutes|m|h|hr|hrs|hour|hours|saat|dakika|stunde|stunden|minuto|minutos|hora|horas|heure|heures|ora|ore|час|часа|часов|минута|минуты|минут|時間|分|小时|分鐘|시간|분|ساعة|ساعات|دقيقة|دقائق|घंटा|घंटे|मिनट|uur|uren|godzina|godziny|minuta|minuty|timme|timmar|jam|menit|giờ|phút)$/iu;
const genericQualityPattern = /^(?:(?:2k|4k|8k|12k|hd|full\s*hd|uhd|hdr|sdr|dolby\s*vision|\d{2,3}\s*fps|pov)[\s|+,&/-]*)+$/iu;
const promotionalLabelPattern = /\b(?:advertisement|channel|follow|instagram|like|merch|patreon|promo|promotion|sponsor|subscribe|tiktok|youtube)\b/iu;
...
export function isGenericLocationLabel(value: string) {
  const normalized = normalizeEvidenceLabel(value);
  const words = normalized.split(/\s+/).filter(Boolean);

  if (!normalized) return true;
  if (genericLocationLabels.has(normalized)) return true;
  if (genericDurationPattern.test(normalized)) return true;
  if (genericQualityPattern.test(normalized)) return true;
  if (promotionalLabelPattern.test(normalized)) return true;
  if (/^(?:part|chapter|section|day)\s+\d+$/iu.test(normalized)) return true;
  if (/^(?:\d+|\d+[a-z]?)$/iu.test(normalized)) return true;

  return words.length > 0 && words.every((word) =>
    genericLabelWords.has(word) ||
    /^\d+(?:[.,]\d+)?$/u.test(word) ||
...
function isSafeProperName(value: string) {
  ...
  if (isGenericLocationLabel(cleaned)) {
    return false;
  }
...
const PREFERRED_META_DESCRIPTION_LENGTH = 180;
const MAX_META_DESCRIPTION_LENGTH = 250;
...
const conciseDescriptionClosingText: Record<
  Lang,
  { withQuality: string; basic: string }
> = {
  en: { withQuality:'The recording presents this {feature} from {location} in {quality}, highlighting the visible places, surroundings and activity.', basic:'The recording presents this {feature} from {location}, highlighting the visible places, surroundings and activity.' },
  tr: { withQuality:'Kayıt, {location} çevresindeki bu {feature} içeriğini; görünür mekânları, çevre ayrıntılarını ve hareketliliği öne çıkararak {quality} görüntü kalitesiyle sunuyor.', basic:'Kayıt, {location} çevresindeki bu {feature} içeriğini; görünür mekânları, çevre ayrıntılarını ve hareketliliği öne çıkararak sunuyor.' },
  de: { withQuality:'Die Aufnahme präsentiert diesen {feature} aus {location} in {quality} und hebt sichtbare Orte, die Umgebung und Aktivitäten hervor.', basic:'Die Aufnahme präsentiert diesen {feature} aus {location} und hebt sichtbare Orte, die Umgebung und Aktivitäten hervor.' },
  es: { withQuality:'La grabación presenta este {feature} de {location} en {quality}, destacando los lugares, el entorno y la actividad visibles.', basic:'La grabación presenta este {feature} de {location}, destacando los lugares, el entorno y la actividad visibles.' },
  it: { withQuality:'La registrazione presenta questo {feature} da {location} in {quality}, mettendo in evidenza i luoghi, l’ambiente e le attività visibili.', basic:'La registrazione presenta questo {feature} da {location}, mettendo in evidenza i luoghi, l’ambiente e le attività visibili.' },
  fr: { withQuality:'L’enregistrement présente ce {feature} de {location} en {quality}, en mettant en valeur les lieux, l’environnement et l’activité visibles.', basic:'L’enregistrement présente ce {feature} de {location}, en mettant en valeur les lieux, l’environnement et l’activité visibles.' },
  ja: { withQuality:'映像は{location}のこの{feature}を{quality}で紹介し、実際に見える場所、周辺環境、動きを伝えます。', basic:'映像は{location}のこの{feature}を紹介し、実際に見える場所、周辺環境、動きを伝えます。' },
  pt: { withQuality:'A gravação apresenta este {feature} de {location} em {quality}, destacando os locais, o ambiente e as atividades visíveis.', basic:'A gravação apresenta este {feature} de {location}, destacando os locais, o ambiente e as atividades visíveis.' },
  ru: { withQuality:'Запись представляет этот {feature} из {location} в качестве {quality}, показывая видимые места, окружение и активность.', basic:'Запись представляет этот {feature} из {location}, показывая видимые места, окружение и активность.' },
  zh: { withQuality:'画面以{quality}呈现{location}的这段{feature}，突出可见地点、周边环境与活动。', basic:'画面呈现{location}的这段{feature}，突出可见地点、周边环境与活动。' },
  ko: { withQuality:'이 영상은 {location}의 {feature}을(를) {quality} 화질로 보여 주며, 실제 장소와 주변 환경 및 활동을 담습니다.', basic:'이 영상은 {location}의 {feature}을(를) 보여 주며, 실제 장소와 주변 환경 및 활동을 담습니다.' },
  ar: { withQuality:'يقدم التسجيل هذا {feature} من {location} بجودة {quality} مع إبراز الأماكن والبيئة والحركة الظاهرة.', basic:'يقدم التسجيل هذا {feature} من {location} مع إبراز الأماكن والبيئة والحركة الظاهرة.' },
  hi: { withQuality:'रिकॉर्डिंग {location} के इस {feature} को {quality} गुणवत्ता में प्रस्तुत करती है और दिखाई देने वाले स्थानों, परिवेश तथा गतिविधियों को प्रमुखता देती है।', basic:'रिकॉर्डिंग {location} के इस {feature} को प्रस्तुत करती है और दिखाई देने वाले स्थानों, परिवेश तथा गतिविधियों को प्रमुखता देती है।' },
  nl: { withQuality:'De opname presenteert deze {feature} uit {location} in {quality} en belicht de zichtbare locaties, omgeving en activiteit.', basic:'De opname presenteert deze {feature} uit {location} en belicht de zichtbare locaties, omgeving en activiteit.' },
  pl: { withQuality:'Nagranie przedstawia ten {feature} z {location} w jakości {quality}, podkreślając widoczne miejsca, otoczenie i aktywność.', basic:'Nagranie przedstawia ten {feature} z {location}, podkreślając widoczne miejsca, otoczenie i aktywność.' },
  sv: { withQuality:'Inspelningen presenterar denna {feature} från {location} i {quality} och lyfter fram synliga platser, omgivningar och aktiviteter.', basic:'Inspelningen presenterar denna {feature} från {location} och lyfter fram synliga platser, omgivningar och aktiviteter.' },
  id: { withQuality:'Rekaman ini menyajikan {feature} dari {location} dalam kualitas {quality}, dengan menyoroti tempat, lingkungan, dan aktivitas yang terlihat.', basic:'Rekaman ini menyajikan {feature} dari {location}, dengan menyoroti tempat, lingkungan, dan aktivitas yang terlihat.' },
  vi: { withQuality:'Bản ghi trình bày {feature} tại {location} ở chất lượng {quality}, làm nổi bật các địa điểm, khung cảnh và hoạt động có thể quan sát.', basic:'Bản ghi trình bày {feature} tại {location}, làm nổi bật các địa điểm, khung cảnh và hoạt động có thể quan sát.' }
};

const metaSummaryText: Record<Lang, string[]> = {
  en: [
    'Watch the embedded recording and review the verified route, publication and location details before choosing another destination.',
    'Use the page details to compare this recording with other verified videos in the collection.'
  ],
  tr: [
    'Gömülü kaydı izleyin; başka bir destinasyon seçmeden önce doğrulanmış rota, yayın ve konum ayrıntılarını inceleyin.',
    'Bu kaydı koleksiyondaki diğer doğrulanmış videolarla sayfa bilgileri üzerinden karşılaştırın.'
  ],
  de: [
    'Sehen Sie die eingebettete Aufnahme an und prüfen Sie bestätigte Angaben zu Route, Veröffentlichung und Ort.',
    'Vergleichen Sie diese Aufnahme anhand der Seitendaten mit weiteren bestätigten Videos der Sammlung.'
  ],
  es: [
    'Mira la grabación insertada y consulta los datos verificados de ruta, publicación y ubicación antes de elegir otro destino.',
    'Compara esta grabación con otros vídeos verificados de la colección mediante los datos de la página.'
  ],
  it: [
    'Guarda la registrazione incorporata e consulta i dati verificati su percorso, pubblicazione e luogo.',
    'Confronta questa registrazione con altri video verificati della raccolta usando i dati della pagina.'
  ],
  fr: [
    'Regardez l’enregistrement intégré et consultez les données vérifiées sur le parcours, la publication et le lieu.',
    'Comparez cet enregistrement aux autres vidéos vérifiées de la collection à partir des informations de la page.'
  ],
  ja: [
    '埋め込み映像を再生し、次の目的地を選ぶ前に、確認済みのルート、公開情報、撮影場所のデータを確認できます。',
    'ページに表示された情報を使って、この映像をコレクション内のほかの確認済み映像と比較できます。'
  ],
  pt: [
    'Assista à gravação incorporada e consulte os dados verificados de rota, publicação e localização.',
    'Compare esta gravação com outros vídeos verificados da coleção usando as informações da página.'
  ],
  ru: [
    'Посмотрите встроенную запись и проверьте подтверждённые сведения о маршруте, публикации и месте съёмки.',
    'Сравните эту запись с другими проверенными видео коллекции по данным на странице.'
  ],
  zh: [
    '播放嵌入视频，并在选择下一个目的地前查看已经确认的路线、发布时间和拍摄地点信息。',
    '您可以依据页面中的可靠数据，将这段视频与合集内其他已确认的视频进行比较，并根据页面显示的地点与类别信息选择下一段内容。'
  ],
  ko: [
    '삽입된 영상을 재생하고 다음 목적지를 선택하기 전에 확인된 경로, 게시 정보와 촬영 위치를 살펴보세요.',
    '페이지의 신뢰할 수 있는 정보를 이용해 컬렉션의 다른 확인된 영상과 비교할 수 있습니다.'
  ],
  ar: [
    'شاهد التسجيل المضمّن وراجع بيانات المسار والنشر والموقع التي تم التحقق منها قبل اختيار وجهة أخرى.',
    'قارن هذا التسجيل بالفيديوهات الموثقة الأخرى في المجموعة باستخدام معلومات الصفحة.'
  ],
  hi: [
    'एम्बेड किया गया वीडियो देखें और अगला गंतव्य चुनने से पहले सत्यापित मार्ग, प्रकाशन और स्थान की जानकारी जाँचें।',
    'पेज पर उपलब्ध विश्वसनीय जानकारी से इस रिकॉर्डिंग की तुलना संग्रह के अन्य सत्यापित वीडियो से करें।'
  ],
  nl: [
    'Bekijk de ingesloten opname en controleer de geverifieerde route-, publicatie- en locatiegegevens.',
    'Vergelijk deze opname via de paginagegevens met andere geverifieerde video’s in de collectie.'
  ],
  pl: [
    'Obejrzyj osadzone nagranie i sprawdź zweryfikowane dane trasy, publikacji oraz miejsca.',
    'Porównaj to nagranie z innymi zweryfikowanymi filmami w kolekcji na podstawie danych strony.'
  ],
  sv: [
    'Se den inbäddade inspelningen och kontrollera verifierade uppgifter om rutt, publicering och plats.',
    'Jämför inspelningen med andra verifierade videor i samlingen med hjälp av sidans uppgifter.'
  ],
  id: [
    'Tonton rekaman tersemat dan periksa rincian rute, publikasi, serta lokasi yang telah diverifikasi.',
    'Bandingkan rekaman ini dengan video terverifikasi lain dalam koleksi melalui data halaman.'
  ],
  vi: [
    'Xem video được nhúng và kiểm tra thông tin đã xác minh về tuyến đường, ngày đăng và địa điểm.',
    'Dùng dữ liệu trên trang để so sánh video này với các video đã xác minh khác trong bộ sưu tập.'
  ]
};
  ...
  return (video.chapters || [])
    .filter(
      (chapter) =>
        ...
        !isGenericLocationLabel(chapter.title)
    )
    .map((chapter) => ({
      ...chapter,
  ...
  return Array.from(
    new Set(
      (video.routePoints || [])
        .filter((point) =>
          point.verified === true &&
          point.geoVerified === true &&
          point.sourceVerified !== false &&
          !isGenericLocationLabel(point.name)
        )
...
export function getVideoDescriptionParagraphs(
  ...
  const firstParagraph = cleanGeneratedText(
    [overview, ...routeFacts].filter(Boolean).join(' ')
  );
  ...
  const closingTemplate = conciseDescriptionClosingText[l];
  const closing = location
    ? fillTemplate(
        quality ? closingTemplate.withQuality : closingTemplate.basic,
        { location, quality, feature: localizedFeature }
      )
    : '';
  const secondParagraph = cleanGeneratedText(
    [...sourceFacts, closing].filter(Boolean).join(' ')
  );

  return [firstParagraph, secondParagraph].filter(Boolean).slice(0, 2);
}
...
function trimMetaDescription(value: string) {
  ...
  if (cleaned.length <= PREFERRED_META_DESCRIPTION_LENGTH) {
    return cleaned;
  }

  const limit = Math.min(
    PREFERRED_META_DESCRIPTION_LENGTH,
    MAX_META_DESCRIPTION_LENGTH
  );
  const limited = cleaned.slice(0, limit - 1);
  ...
  const minimumSentenceLength = MIN_META_DESCRIPTION_LENGTH;
  ...
  const trimmed = wordEnd >= MIN_META_DESCRIPTION_LENGTH
...
export function getVideoMetaDescription(
  ...
  const l = normalizeLang(lang);
  const analysis = getVideoSeoAnalysis(video, l);
  const feature = getSeoStyleLabel(
    analysis.style,
    l,
    video.id,
    Boolean(analysis.detail)
  ...
  const duration = formatLocalizedDuration(video.duration, l)
    .replace(/[.。]+$/u, '')
    .trim();
  const sourceText = publisher && published
    ? fillTemplate(descriptionSentenceTemplates[l].sourceDate, {
        publisher,
        date: published
      })
    : publisher
      ? fillTemplate(descriptionSentenceTemplates[l].sourceOnly, { publisher })
      : published
        ? fillTemplate(descriptionSentenceTemplates[l].dateOnly, { date: published })
        : '';
  const parts = [
    location
      ? `${location} — ${[quality, feature].filter(Boolean).join(' ')}.`
      : `${[quality, feature].filter(Boolean).join(' ')}.`,
    analysis.detail
      ? formatLabeledFact(labels.featured, analysis.detail)
      : '',
    sourceText,
    duration
      ? fillTemplate(descriptionSentenceTemplates[l].duration, { duration })
      : '',
    metaSummaryText[l][0]
  ].filter(Boolean);
  ...
  if (hotels.length) {
    ...
    const hotelText = formatLabeledFact(
      labels.nearbyHotels,
      nearest.distanceMeters === undefined
        ? nearest.name
        : `${nearest.name} (${formatLocalizedDistance(nearest.distanceMeters, l)})`
    );
    if (cleanGeneratedText([...parts, hotelText].join(' ')).length <= 210) {
      parts.push(hotelText);
    }
  }

  if (cleanGeneratedText(parts.join(' ')).length < MIN_META_DESCRIPTION_LENGTH) {
    parts.push(metaSummaryText[l][1]);
  }
...
export function getVideoH1(
  ...
  const durationQualifier = formatLocalizedDuration(video.duration, l);
  const qualifier = [dateQualifier, durationQualifier]
    .filter((value) => value && !baseTitle.includes(value))
    .filter(Boolean)
    .join(' · ');

  if (
    !qualifier ||
    baseTitle.includes(qualifier)
  ) {
    return baseTitle;
  }
