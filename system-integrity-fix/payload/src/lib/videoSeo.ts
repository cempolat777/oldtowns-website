...
export const VIDEO_CONTENT_VERSION = 8;
...
const weakWords = new Set([
  ...
  'feels','feel','looks','look','hour','hours','hr','hrs','long',
  'minute','minutes','complete','entire','true','full',
...
function isMeaningfulEvidenceLabel(value: string) {
  const normalized = normalizeEvidenceLabel(value);

  if (!normalized || genericEvidenceLabels.has(normalized)) {
    return false;
  }
...
function hasTrustedVideoGeo(video: Video) {
  return (
    video.geo?.verified === true &&
    video.geo?.integrityVerified === true &&
    typeof video.geo?.latitude === 'number' &&
    Number.isFinite(video.geo.latitude) &&
    typeof video.geo?.longitude === 'number' &&
    Number.isFinite(video.geo.longitude) &&
    typeof video.geo?.cityDistanceMeters === 'number' &&
...
export function getVideoH1(
  ...
  const l = normalizeLang(lang);
  const titleDate = formatSeoTitleDate(
    video.publishedAt,
    l
  );
